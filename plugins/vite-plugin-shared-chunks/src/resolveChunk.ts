...
/** antdv 内核：与 runtime 循环依赖的模块必须同 chunk，否则运行时报 Cannot access before initialization */
...
/** antdv-next 按 dist 子目录拆包：core（运行时）+ 各业务组件独立 chunk */
...
/** 子应用 consumer 按导出名解析 chunk；core 内模块统一走 core.js */
...
/** 工作区包体量小，保持单文件 shared/wc-ui、shared/wc-utils */
...
export const defaultResolveChunkName = (packageName: string, moduleId: string): string | null => {
    ...
    if (packageName === 'wc-ui' || packageName === 'wc-utils') {
...
export const resolveSharedChunkFileKey = (
    ...
    if (bareSpecifier === 'wc-ui' || bareSpecifier === 'wc-utils') {
